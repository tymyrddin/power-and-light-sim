...
for i in range(5):
    try:
        ...
        holding_response = client.read_holding_registers(address=0, count=1, device_id=1)
        ...
        if not holding_response.isError() and not input_response.isError():
            ...
            match = "YES" if holding_val == input_val else f"NO (diff: {abs(holding_val - input_val)})"
